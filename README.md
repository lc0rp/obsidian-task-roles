...
- **Quick shortcuts**: Type `\d`, `\a`, `\c` or `\i` to insert role fields. Custom roles can define their own letter. Inside `tasks` or `taskview` code blocks, the shortcut inserts `🚗 = ` style markup.
