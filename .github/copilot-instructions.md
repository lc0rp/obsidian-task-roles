...
### Linting Commands - IMPORTANT CONFIGURATION ISSUE
```bash
# CRITICAL: ESLint has a configuration issue with flat config format
# The npm scripts fail but the workaround below works correctly
# Use this workaround for TypeScript linting:
ESLINT_USE_FLAT_CONFIG=true npx eslint -c config/eslint.config.js src

# Standard commands that WILL FAIL due to configuration issues:
npm run lint:ts        # FAILS - use workaround above
npm run lint:ts:fix    # FAILS - use workaround above with --fix flag

# Markdown linting works correctly:
npm run lint:md        # Works - takes ~1 second
npm run lint:md:fix    # Works - takes ~1 second

# Combined linting (will fail due to ESLint issue):
npm run lint           # FAILS - run components separately  
npm run lint:fix       # FAILS - run components separately
```
...
1. Use ESLint workaround: `ESLINT_USE_FLAT_CONFIG=true npx eslint -c config/eslint.config.js src`
...
4. Run linting: `ESLINT_USE_FLAT_CONFIG=true npx eslint -c config/eslint.config.js src`
...
**Problem**: ESLint 8.x doesn't support flat config format by default, causing npm scripts to fail
**Workaround**: Use `ESLINT_USE_FLAT_CONFIG=true` environment variable with direct npx commands
**Commands**:
```bash
# For linting:
ESLINT_USE_FLAT_CONFIG=true npx eslint -c config/eslint.config.js src
...
# For auto-fix:
ESLINT_USE_FLAT_CONFIG=true npx eslint -c config/eslint.config.js src --fix

# DO NOT USE npm run lint:ts or npm run lint - they will fail
```
