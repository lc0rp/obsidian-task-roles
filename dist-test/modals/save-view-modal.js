...
export class SaveViewModal extends Modal {
    constructor(app, viewConfigService, layout, filters, sortBy, onSave) {
        super(app);
    ...
    onOpen() {
        ...
        saveButton.onclick = async () => {
            ...
            try {
                ...
                const result = await this.viewConfigService.saveViewConfiguration(viewName.trim(), this.layout, this.filters, this.sortBy, exists // Allow overwrite if name exists
                );
                if (result.success) {
    ...
    updateOverwriteWarning(exists) {
        const warning = this.contentEl.querySelector('.overwrite-warning');
        if (warning) {
