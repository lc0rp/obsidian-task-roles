...
export class AssignmentModal extends Modal {
    ...
    renderAssignees(container, roleId, assignees) {
        let assigneeContainer = container.querySelector('.assignee-container');
        ...
        for (const assignee of assignees) {
            ...
            assigneeEl.onclick = (e) => {
                ...
                this.renderAssignees(container, roleId, this.assignments.find(a => a.roleId === roleId)?.assignees || []);
            };
    ...
    showAssigneeSelector(roleId, container) {
        new AssigneeSelectorModal(this.app, this.plugin, async (assignee) => {
    ...
    removeAssignee(roleId, assignee) {
        const assignment = this.assignments.find(a => a.roleId === roleId);
        if (assignment) {
    ...
    applyAssignments() {
        const currentLine = this.editor.getLine(this.editor.getCursor().line);
        const newLine = this.plugin.taskAssignmentService.applyAssignmentsToLine(currentLine, this.assignments, this.plugin.getVisibleRoles());
