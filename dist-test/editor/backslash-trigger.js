...
export function backslashTrigger(app, settings) {
    return ViewPlugin.fromClass(class {
        ...
        openRoleSelector() {
            ...
            // Get visible roles
            const visibleRoles = settings.roles.filter(role => !role.isDefault || !settings.hiddenDefaultRoles.includes(role.id));
            // Create a simple popup with role options
            this.showRolePopup(visibleRoles, editor, cursor, isInTaskBlock);
        }
        showRolePopup(roles, editor, cursor, isInTaskBlock) {
      ...
      `;
        ...
        `;
            ...
            const keyHandler = (e) => {
                if (e.key === 'Escape') {
                    ...
                    options[selectedIndex].style.backgroundColor = '';
                    selectedIndex = (selectedIndex + 1) % options.length;
                    options[selectedIndex].classList.add('selected');
                    options[selectedIndex].style.backgroundColor = 'var(--background-modifier-hover)';
                }
        ...
        selectRole(role, editor, cursor, isInTaskBlock) {
