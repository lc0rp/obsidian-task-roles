...
export class ExpandableFiltersComponent {
    ...
    render(container: HTMLElement): void {
        const filtersEl = container.createDiv('task-roles-filters');

        // Filter toggle
        const filterToggle = filtersEl.createEl('button', { cls: 'task-roles-filter-toggle' });
        setIcon(filterToggle, 'filter');
        filterToggle.setText('Filters');

        // Add arrow icon
        const arrowIcon = filterToggle.createSpan('task-roles-filter-arrow');
        setIcon(arrowIcon, 'chevron-down');

        const filtersContent = filtersEl.createDiv('task-roles-filters-content');
    ...
    private renderFilterControls(container: HTMLElement): void {
        const filterGrid = container.createDiv('task-roles-filter-grid');
        ...
        const assigneesInput = assigneesGroup.createEl('input', {
            ...
            cls: 'task-roles-assignees-input'
        });
        ...
        const dateTypeContainer = dateGroup.createDiv('task-roles-layout-container');
        const dateTypeSelect = dateTypeContainer.createEl('select', { cls: 'task-roles-date-type-select' });
        const dateTypes = [
        ...
        const dateTypeArrow = dateTypeContainer.createSpan('task-roles-dropdown-arrow');
        ...
        const filterActionsEl = filterGrid.createDiv('task-roles-filter-actions');

        // Clear filters button (first in order)
        const clearFiltersBtn = filterActionsEl.createEl('button', { cls: 'task-roles-clear-filters-btn' });
        ...
        const cancelFiltersBtn = filterActionsEl.createEl('button', { cls: 'task-roles-cancel-filters-btn' });
        ...
        const applyFiltersBtn = filterActionsEl.createEl('button', { cls: 'task-roles-apply-filters-btn' });
        ...
        const autoApplyContainer = filterActionsEl.createDiv('task-roles-auto-apply-container');
        const autoApplyCheckbox = autoApplyContainer.createEl('input', {
            type: 'checkbox',
            cls: 'task-roles-auto-apply-checkbox'
        });
        autoApplyCheckbox.checked = this.plugin.settings.autoApplyFilters;

        const autoApplyLabel = autoApplyContainer.createEl('label', { cls: 'task-roles-auto-apply-label' });
